...
carStep staticL dyn (xa,ya) (Car idC (xd,yd) pathOld col) = 
    ...
    where --next = nextField staticL (xd,yd) (xa,ya)
          next = nextField staticL (getCell staticL (xa,ya)) (xd,yd) pathOld 
...
---------------------------- < pathfinding > ----------------------------------
{-
nextField :: [[Cell]] -> Pos -> Pos -> Pos
nextField staticC destination position = 
    if length path > 1 
...
--          (fOld,_) = head nodeInList
--          nodeInList :: Pos -> [(Int, Tree Pos)]
--          nodeInList posSuc = filter (\(_, Node pos _) -> pos==posSuc)
--                          openList
--          withoutNode :: Pos -> [(Int, Tree Pos)]
--          withoutNode posSuc = filter (\(_, Node pos _) -> pos/=posSuc)
--                                   openList
